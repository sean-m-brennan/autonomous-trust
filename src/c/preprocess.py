...
def preprocess(target_filepath: str, output_file: str, directory: str, rel_path: str = None):
    ...
    with open(target_filepath, 'r') as f:
        ...
        for idx, line in enumerate(list(lines)):
            ...
            elif line.startswith('#define') and 'DECLARE_' in line:
                i = line.find('DECLARE_')
                j = line.find('(', i+1)
                delimiter = line[i:j]
    ...
    if 'QUOTE' in definition:
        ...
        definition = definition.replace('QUOTE(', '"')
    ...
    for decl in declarations:
        ...
        if len(args) != len(decl_tokens):
            print("Mismatched declaration/definition")
            sys.exit(-1)
