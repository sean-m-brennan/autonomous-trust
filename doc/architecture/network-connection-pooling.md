...
2026-08-10. With `AT_NET_POOL=0` the transport behaves exactly as it always has,
one `connect`/`send`/`close` per message.
...
application message, meaning `socket()`, `connect()`, one length-prefixed frame,
then `close()`. The receiver mirrors it, accepting a client socket, reading
exactly one frame, then dropping the socket. Every message therefore costs a
full handshake and teardown on both ends. The cost lands on setup, teardown, and
...
payload (`struct.pack('!I', len)` + body). A pooled connection simply carries
many of these frames back to back. `_recv` already reads exactly one frame at a
time, so a persistent reader is the same read logic called in a loop. Because
the bytes on the wire are identical either way, the conformance corpus (which
pins message bytes, not connection lifecycle) is unaffected.
...
Three behaviors are worth calling out, namely reconnect, keepalive, and
reaping.

**Reconnect once.** A pooled socket can be half-open, since the peer recycled or
reset it while this side has not noticed yet. The first write on such a socket fails
...
wakeups. That is the escape hatch if a much larger cohort ever needs it, and it is
not implemented today.

## Concurrency

The send pool is guarded by one lock. Sends originate on the network main loop,
so the job of the lock is to keep the group and ping paths from racing the loop,
not to arbitrate many concurrent senders. Receive readers are independent, one per
...
the same reason. At shutdown, `close_connections` drains the send pool, and reader
...
- A pooling sender talking to a receiver that takes **one frame per connection**
 ...
 and always was. The length prefix and body of the frame go out as two `send`s,
 the RST from the first is seen by the second, and the reconnect-once retry in
 `_send_tcp_pooled` re-sends the whole frame. Measured 20/20 delivered. The
 ...
 which both runtimes now do. In the same four-frame scenario against the fixed
 C receiver, 4/4 were delivered over one connection.
...
Both runtimes now reuse on the receive side. The C transport
...
senders still connect per message, which is a missed optimization rather than a
...
> and the receiver sees well-formed frames, and the messages simply are not
> there.
...
(`net_conn_idle_ttl_resolve` / `net_max_live_conns_resolve` in `net_proc.c`), so
one deployment setting tunes both runtimes.
...
`net.tcp.send/reuse` dominating `net.tcp.send/connect` is the signal to look
for.
...
rate should fall well below the message rate. The counters to watch are reuse,
...
- **Resource exhaustion.** Long-lived sockets widen the surface for a peer that
 ...
 still apply before a reader thread is ever created. The bounds are the cap,
 the TTL, and the gates.
