...
A technical introduction: motivations, philosophy, and the mechanisms.
...
- Every decision is made **at the node** — no round-trip to a central policy engine or PKI.
...
Zero Trust as normally built (NIST SP 800-207) is **policy-driven → human-driven** and assumes a **reachable central authority** (CA / OCSP / CRL) on the hot path of *every* connection. That premise fails three ways:

| Failure mode | Why it happens |
| --- | --- |
| **Dies when comms drop (DDIL)** | Unreachable policy engine → no new connection. CRLs are tens of MB over kbps links; Mars light-time makes cert rotation take *hours*. Result: **fail-open or fail-closed.** |
...
- AT replaces that with a **continuous behavioral gradient**:
  ...
  - A valid credential is a prerequisite to be *heard* — never a substitute for *trust*.
...
- **Zero-trust default-deny.** Every peer starts untrusted; nothing is implied by identity alone.
- **Earned & emergent.** Standing accrues from observed behavior in an iterated game — not conferred by fiat.
- **Conferred by peers, weighted by *their* standing** — a PageRank-like web of opinion, not a central verdict.
- **Adversarial-always.** Assume any peer may be hostile; **least privilege**, task-scoped, bounds the blast radius.
- **Fail-safe, not fail-open.** Uncertainty degrades access gracefully; it never silently grants it.
- A **human operator is just a (privileged) peer** — the same machinery, with PIV/CAC + MFA.
...
1. **Refuse traffic** from badly-trusted peers → save **bandwidth**
2. Accept traffic but **refuse compute** to weakly-trusted peers → protect **CPU**
3. Offer compute but **withhold data** from moderately-trusted peers → protect **data**
...
**Decentralized by construction** — groups form organically, reputation is maintained by leaderless Byzantine Multi-Paxos, tasks are negotiated peer-to-peer. Four cooperating subsystem processes:
...
- **Two separate key pairs per identity**: an Ed25519 **signing** pair (one-to-many verify) and an X25519 **encryption** pair (one-to-one Box). Private material stays local; `publish()` yields a public-only copy.
- **Zooko-style naming** (decentralized, human-meaningful, secure — pick all three):
  - `nickname` = **online** global name (e.g. `squad-warrant@dod-demo`) — the only name on the wire.
  - `petname` = **local** name each receiver mints for itself — **never serialized**, so no one can inject into your naming.
- Stable **UUID** per identity; duplicate UUID / signing-key / encryption-key collisions are rejected on sight.
- **Canonical wire form** is a flat JSON that is **byte-identical between the Python and C implementations** (the interoperability contract).

---

## Identity: a verifiable ledger, permissionless admission

- **Identity history is a DAG of Merkle roots** — think *git*: identity blobs are diffs, Merkle roots are commits, divergent branches merge at their lowest common ancestor. Membership is provable (Merkle inclusion proofs); backdating is rejected by timestamp monotonicity.
- **Admission is permissionless but Sybil-resistant** (border-guard pattern):
...
- **Admission agreement is pluggable**: Proof of Work / Proof of Stake / **Proof of Authority**.
- Known-UUID peers (amnesia / restart) skip the vote; group key rotates on membership change.

---

## Reputation I: the transaction ledger

Reputation rests on a **hash-linked, bilateral transaction chain** — not a global blockchain, but a bounded, per-node verifiable commit log.

- Every `Transaction` is intrinsically **bilateral** (two counterparties, two scores); one peer can't fill both sides.
- **Hash-linking (blake2b `prev_hash`)** chains committed entries → a tampered segment is *detectable*, so peers sync by **verifying, not trusting**.
- **Merkle window root** (RFC 6962, domain-separated to defeat CVE-2012-2459) gives O(log n) inclusion audit.
- **Signed checkpoints** = a quorum co-signs a window root → a lightweight **finality gadget**.
...
Scores live in **[0, 1]** — **no negatives.** Single-source thresholds (env-overridable):
...
- **Two modes with hysteresis** (band `[0.45, 0.55]` retains the prior mode):
  - **Cooperation** = *pure reputation*: trust-weighted average of counterparties' scores × their standing × task weight.
  - **Contrite Tit-for-Tat** = principled response to a counterparty's defection against *you*.
- **Idle decay is asymmetric**: idle peers relax toward 0.21 but decay **never rehabilitates** a low/slashed peer.
...
**Agreement on reputation = leaderless Byzantine Multi-Paxos** — no coordinator to capture or lose.
...
- **Catch-up** queries the top-3 most-trusted peers and majority-votes the result (verifiable via chain links).
- **Slashing** is the fast path: a **quorum-co-signed (Ed25519), evidence-gated** `SlashAttestation` **floors** a score immediately — no waiting ~20 transactions for the average to move. Handles short-lived rogues.
- **Exclusion is sticky**: a cut-off peer can't transact its way back; recovery is an explicit, signed `rehabilitate`.
...
- **This is where trust composes**: the requester verifies the result's ZKP, then submits a transaction score → feeds Reputation.
...
- **One agreement framework, two voter bases** (proofs are Ed25519-signed, tallied by a threshold of the top third):
  - **Proof of Authority** — votes weighted by **rank** (topology/capability).
  - **Proof of Trust** — votes weighted by **tier** (earned reputation). Use it for behavior-dependent decisions like data-sharing.
- **The cold-start problem**: how does a brand-new mesh earn *any* reputation? A **bootstrap capability corpus** — auto-registered on every node:
...
- **Three channels, by trust level:**
  1. **Open broadcast** (UDP, unencrypted) — *announcements only*
  2. **Encrypted group** (NaCl SecretBox, shared key) — proposals, votes, Paxos
  3. **Encrypted peer-to-peer** (NaCl Box) — history, negotiation, reputation
- **UDP / TCP hybrid** transports (length-prefixed TCP framing); optional connection pooling — wire bytes unchanged, so C and Python nodes interoperate.
...
**AT extends NIST SP 800-207 — it does not replace it.** A peer still needs valid credentials to be heard; behavioral reputation is the operative boundary *after* connection.

- **Pluggable verifier interface** (`verify` / `check_revocation` / `is_available`), build-gated by `AT_ZTA`:
  - **X.509 verifier** (OpenSSL / pyca) — chain + expiry, optional OCSP/CRL. *Shipped, both implementations.*
  - **OIDC verifier** — interface present, *stub* today.
- **Credential bound to identity** by SHA-256 hash, **excluded from identity equality** → routine cert rotation never disturbs reputation.
...
- **Earned, never advertised**: a wire claim is neutralized at admission and set true only if a *distinct operator credential* verifies.
- Operator access = **PIV/CAC + MFA** (TOTP default, DDIL-friendly); key never leaves the card; fail-safe.

**Behavioral anomaly layer** *(Python prototype)*
- Online, explainable detectors (River HST + PyOD HBOS), **deterministic** → scores are admissible signed evidence.
- **"ML proposes, consensus disposes"**: a per-node governor emits a *slash proposal*; the signed quorum flow decides.
- **Human-on-the-loop by default**; dwell + quorum guards defeat the base-rate fallacy.
...
- runs the embedded/ARM daemon (`at_demo`), no Python on-device
- **formally verified** (Frama-C / ACSL contracts): 100% of attempted proof goals discharged

</div>

- Wire-interoperable; backend chosen at import (`AUTONOMOUS_TRUST_BACKEND = auto | native | python`), overlaid per-module via CFFI.
...
| **simulator** | Multi-node mesh with **mobility paths** and radio physics — terrain, delay, routing, even a **space link** (free-space loss, light-delay, occultation) |
...
The **multi-agency disaster-response demo** runs in-process — no Docker required:
...
> The network **excludes it on its own** — locally, continuously, with signed evidence — while the rest keep cooperating.
