...
def generate_full_config(filepath: str):
    ...
    peer1 = PeerInfo(str(uuid4()), 'microdrone', '192.168.0.2', path1.shape.start + 1, -200.,
    ...
    peer2 = PeerInfo(str(uuid4()), 'microdrone', '192.168.0.3', path2.shape.start + 2, -200.,
    ...
    peer3 = PeerInfo(str(uuid4()), 'microdrone', '192.168.0.4', path3.shape.start + 3, -200.,
    ...
    peer4 = PeerInfo(str(uuid4()), 'microdrone', '192.168.0.5', path4.shape.start + 4, -200.,
    ...
    peer0 = PeerInfo('df5a182d-852e-4481-bd97-22218e732368', 'recon', '192.168.0.1', path0.shape.start, -1200.,
...
def generate_small_config(filepath: str):
    ...
    shape1 = BezierData(t5, uah, [one, two])
    path1 = PathData(start, end, shape1, Variability.GAUSSIAN, 4.4, Variability.UNIFORM)
    peer1 = PeerInfo(str(uuid4()), 'microdrone', '192.168.0.2', path1.shape.start, -200.,
                     Antenna.DIPOLE, NetInterface.SMALL, start, end, path1, [])
    shape2 = EllipseData(uah, 1000, 600, -45., 1)
    path2 = PathData(start, end, shape2, Variability.GAUSSIAN, 4.4, Variability.UNIFORM)
    peer2 = PeerInfo(str(uuid4()), 'recon', '192.168.0.1', path2.shape.start, -200.,
                     Antenna.DIPOLE, NetInterface.SMALL, start, end, path2, [])
