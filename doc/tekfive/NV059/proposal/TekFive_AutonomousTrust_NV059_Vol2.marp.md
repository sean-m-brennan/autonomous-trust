...
Zero Trust (NIST SP 800-207) as normally built is **policy-driven → human-driven**, and assumes a **reachable central authority** (CA / OCSP / CRL) on the connection hot path.
...
- Every authenticated peer starts at **zero trust** and earns it through observed behavior.
- Trust is a live value in **[0.0, 1.0]**, evaluated **locally and continuously**, not a one-time boolean.
...
AT is a **resident agent** co-located with each combat-system / OT endpoint, or a **bump-in-the-wire gateway** where a device can't host code.
...
- Sits between the mission application and the network as that node's **policy-decision + enforcement point.**
- **Bounded-memory deterministic C core** + streaming anomaly layer with **fixed per-peer state**, and no data-lake backhaul.
- Runs low-priority co-resident, or **offloads entirely to the gateway** so a compute-starved sensor carries little cost.
- **Phase I characterizes the footprint** (CPU / memory / added latency on embedded ARM) as a first-class result.
...
> AT leads with history-free bases 1 through 4; behavioral ML (base 5) is a **gap-filling refinement**, not the foundation.
...
**Machine / NPE identity** is cryptographic and self-sovereign:
...
**A purpose-built permissioned DLT**, three tuned structures rather than one global chain:

- **Merkle-DAG identity ledger**: who is who.
- **Reputation ledger**: hash-linked, RFC 6962 Merkle-checkpointed, and **slashing-capable.**
- **Audit log**: append-only JSONL, offline-intact, reconciles on reconnect.

> Declines global chain, Proof-of-Work, and unbounded history as the **wrong CAP / resource trade** for a small-memory, intermittently-connected mesh.
...
- Past threshold, slashing triggers **autonomous tier-gate exclusion**: the peer is dropped from the capability level it no longer earns.
...
Each datum & grant carries an **access class = (required_tier, compartment-set)**. The axes are **orthogonal and separately sourced**. A datum can be mission-critical yet uncompartmented, or low-impact yet tightly compartmented.

> Tier-gating is **implemented today**; the compartment axis is layered on **without modifying it** (Phase I design).
...
The topic asks for AI/ML on access patterns. AT's answer: **deterministic enforcement is primary; ML is a subordinate, governed sensor** that feeds evidence to the trust algorithm.

- Each node models the **behavioral envelope** of every peer/role → per-peer **anomaly score with per-feature attribution.**
- A sustained anomaly **feeds reputation**; past threshold it fires the quorum-signed, Merkle-gated **slashing** path above.
- Engine: streaming **Half-Space Trees** + per-feature streaming-histogram detector (**HBOS/LODA**, native attribution).
- **Unsupervised, online, bounded-memory, deterministic under fixed seeds**; prototyped in Python (River/PyOD), reimplemented in the verified C core.
...
- ICAM is kept for human-identity proof but **paid once**, at console activation, **bound to the local AT identity**, off the per-access hot path.
- Nested **PDPs** form a hierarchical, emergent-gateway network; a connected gateway validates via **OCSP stapling / cached short-lived revocation** (pre-staged while connected) and **vouches** through **quorum-gated, reputation-weighted delegated verification.**
- If an adversary shrinks the reachable quorum (jamming / partition): a **sub-threshold quorum admits only at a capped reputation ceiling and reduced tier-gate**: **base-3 containment**, gated by history-free bases 1 and 2.
- **Every admission is signed** → the audit reconciles on reconnect.

> The disconnected operator is verified **through the cohort**, not forced to choose between insecure and mission-kill.
...
- **100% proof on attempted goals (4,262 / 4,262)**, ~**54% function coverage**, with documented skips, scoped to memory-safety and ACSL conformance.
...
| **Captured / spoofed sensor**     | Authenticated & cert-valid, but injects an off-track return →**drifts outside its learned envelope → loses reputation → autonomously slashed and tier-gated out of the fires path, locally, in real time, with signed evidence.** Human-on-the-loop keeps override for safety-critical fires. |
| **SATCOM to PDP / DoD PKI drops** | Cohort keeps operating: targeting node**vouched by quorum at a capped tier**, each admission signed, **audit reconciles on reconnect.**                                                                                                                                                          |
...
Feasibility is established the right way for each claim, with **M&S where simulation is faithful, silicon where it isn't:**

- **M&S** for latency comparison, attack outcomes, admin overhead, against an **externally-anchored ZTA-only baseline** and a **pre-registered, MITRE ATT&CK-mapped attack corpus**, delivered as **reproducible Government artifacts.**
- **Existing subscale C prototype on embedded ARM** for real-time authentication latency; M&S can't substitute for silicon timing.
- Resilience demonstrated under **Sybil, Byzantine, compromised-credential, partition, and DDIL** with no reachable infrastructure; report **false-exclusion rate against a realistic base rate.**
