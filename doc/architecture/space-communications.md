...
The return tuple grows from six to seven elements, adding `DelayMatrix`. Downstream consumers use `SimState` objects (not raw tuples), so the change is transparent to the Dash UI and protocol layers.

## DelayRouter

`DelayRouter` extends the existing `Router` class to inject per-peer-pair latency via Linux `tc netem`. After the parent applies iptables connectivity and tc CBQ bandwidth rules, DelayRouter adds a netem qdisc with the computed light-time delay.
