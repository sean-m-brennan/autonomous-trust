...
is static config in `identity.json`; longer-term dynamic adaptation
is deferred).
...
Add `peer._tier: int` (default 0). Initially this is the new home for
what used to be `peer._rank` populated from reputation. The rename is
mechanical:
...
change re-issues the identity. Dynamic topology adaptation is deferred.
...
**Status (2026-05-22):** weight-aware on both adapters and pinned by
conformance.
...
hard-codes either. Future demos add their own
