*Previous: [Partition recovery](partition-recovery.md)*
...
The AT hierarchy could itself serve as a time source for its cohort. The objection
that peer sources cannot be arranged "at config time" is right about config time
only. Since chrony ≥ 4.2 has `sourcedir` plus `chronyc reload sources`, a node
could write a `.sources` file on admission and reload, adding a peer reference
clock at **runtime** without restarting chrony. Symmetric `peer` mode
would let a cohort discipline itself with no external stratum at all, the
interesting case for a deployment with no route to a public pool.
...
(NTS or symmetric keys) and a bound on how far the cohort may drag a clock. Both
...
timestamps are disciplined by peers selected on reputation, then moving what a
node takes "now" to be moves every comparison that would have detected the move,
...
writes a clock, nothing contacts chrony, and no node depends on the time of
another.

### The measurement

Four timestamps per exchange, with the arithmetic of NTP (RFC 5905 §8).
...
`t1`/`t4` belong to the puller (request sent, response received) and `t2`/`t3`
...
`statistics.median` in Python, so they agree to the bit.
...
runtimes off the float-parsing path. A bad value is refused rather than clamped,
like the other tunables (§2.4.4). 2 s is the pairwise implication of the existing 1 s
...
`clock_sent_at` on the answer. It was already a periodic challenge/response
whose purpose is asking a peer to prove something about itself, so a clock
reading fits its existing meaning, and it is twinned in `id_proc.c`. PingAT was
disqualified for being Python-only, since C answers that selector with
`{"error": "unsupported"}` (§2.4.1). `t1` deliberately never goes on the wire, it is ours,
and a peer echoing it back could lie about it.
...
the AT notion of time diverged from the host it runs on, with no clock
discipline behind it. The offset here is *evidence about a peer*, never a correction to
self.

### Scope of the surfacing

The samples live in the process where the attest round trips complete, the
identity subprocess, and a forked sibling holds its own copy.
`cohort_clock_state()` reports the view held by that process. The measurement
also rides the existing local attestation report to the main loop
(`clock_offset` / `clock_delay` / `clock_usable`), which is where a consumer
reads it. Nothing was added to `NtpTimeSource`, because it runs elsewhere and
cannot see this store.

### Pinning

Unit tests cover both sides (`TestClockSampleMath` / `TestCohortAggregation` /
`TestCohortStaysAdvisory` in `test_network_clock.py`, and the `Clock` suite in
`clock_test.c`), and the round trip is pinned in `test_operator_attestation.py`.
Two conformance scenarios, `cohort-clock-skew-peer-ahead` and `-peer-behind`,
...
`attested_now` and `attest_accepted` are computed by the harness itself, and
they stayed green when a signature change left `handle_attest_response` raising
half-way through, so the corpus could not see it. Assertions that read production
...
sidecar with the chrony `sourcedir` mounted, would validate it, copy it in, and
...
container may steer the clock of its host stays a decision for the host
operator, and an applier small enough to audit is what enforces the policy AT
...
- **Eligibility is provisioned, not earned.** Only a peer holding a
 ...
 (tier-down revokes it) but never add one. This asymmetry is the second clause of the
...
- **Authentication.** NTS fits the existing credential chain (an AT anchor as
 ...
 (`keyfile` plus `server … key <id>`) are easier to provision at runtime. Group-key
 rotation is deliberately deferred, though, so a static key is a long-lived
 cohort-wide shared secret, a fallback for constrained C nodes rather than the
 default.
- **Bounded drag, two-sided.** On the chrony side, `maxchange … panic`, no
 `makestep` for cohort sources, `minsources ≥ 2`, and a worse stratum and poll
 than any external pool so a reachable pool always wins. On the AT side, a
 **cumulative** bound since boot, not only per-step, because the per-step bound
 alone is what slow drag walks through.
...
agree *precisely* on a *wrong* absolute time. That is acceptable for ordering
and fatal for credential validity windows and external log correlation.
Certificate lifetime checks would need to keep reading a source the cohort
cannot influence, or enter an explicit "absolute time unverified" state, never
validate silently against consensus time.
...
1. **Calling `chronyc` from inside the container.** Requires the chrony socket
 mounted, which grants full control over the sources of the host, far more
 than adding one peer.
...
3. **An AT-internal offset applied in `system.now()`.** The defect of the retired
 NTP module, restated. See [Clock discipline](node-lifecycle.md#clock-discipline).

---

*Next: [Persistent cohort](persistent-cohort.md)*
