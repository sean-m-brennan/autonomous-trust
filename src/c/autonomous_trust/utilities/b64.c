...
int32_t base64_encode(const uint8_t *in, size_t data_length, char *result, size_t max_result_length)
{
    ...
    size_t len = b64_encoded_len(data_length);
...
uint32_t b64_pad(size_t size, uint8_t last_byte)
{
    if ((size > 0U) && (((size % 4U) != 0U) || (last_byte == (uint8_t)'=')))
        return 1U;
    return 0U;
}
...
{
    uint32_t pad_uint = b64_pad(size, last_byte);
    const size_t len = (((size + 3U) / 4U) - pad_uint) * 4U;
    return ((len / 4U) * 3U) + pad_uint;
}
...
int32_t base64_decode(const char *in, size_t in_len, uint8_t *out, size_t max_out_len)
{
    ...
    uint32_t pad_uint = b64_pad(in_len,  in_data_uchar[in_len - 1U]);
    const size_t out_len = b64_decoded_len(in_len, in_data_uchar[in_len - 1U]);
